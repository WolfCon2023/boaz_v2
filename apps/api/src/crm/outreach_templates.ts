...
outreachTemplatesRouter.get('/', async (req, res) => {
  ...
  const filter: Record<string, unknown> = q
    ...
    : {}
  const items = await db.collection<TemplateDoc>('outreach_templates').find(filter as any).sort(sort).limit(200).toArray()
...
outreachTemplatesRouter.post('/', async (req, res) => {
  ...
  if (!name || !body) return res.status(400).json({ data: null, error: 'invalid_payload' })
  const now = new Date()
  const doc: any = { name, channel, subject, body, variant: raw.variant || null, createdAt: now, updatedAt: now }
  const result = await db.collection<TemplateDoc>('outreach_templates').insertOne(doc)
...
outreachTemplatesRouter.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (raw.channel === 'sms' || raw.channel === 'email') update.channel = raw.channel
    if (typeof raw.body === 'string') update.body = raw.body
    if (typeof raw.subject === 'string') update.subject = raw.subject
    if (typeof raw.variant === 'string' || raw.variant === null) update.variant = raw.variant
    await db.collection<TemplateDoc>('outreach_templates').updateOne({ _id }, { $set: update })
...
outreachTemplatesRouter.delete('/:id', async (req, res) => {
  ...
  try {
    const _id = new ObjectId(req.params.id)
    await db.collection<TemplateDoc>('outreach_templates').deleteOne({ _id })
    res.json({ data: { ok: true }, error: null })
  } catch {
    res.status(400).json({ data: null, error: 'invalid_id' })
  }
