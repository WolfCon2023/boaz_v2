...
function serializeTemplate(doc: ContractTemplateDoc) {
  return {
    ...doc,
    _id: String(doc._id),
...
contractTemplatesRouter.get('/', async (req, res) => {
  ...
  const items = await db
    .collection<ContractTemplateDoc>('contract_templates')
    .find(filter)
    .sort({ updatedAt: -1 })
    .limit(200)
    .toArray()
...
contractTemplatesRouter.post('/', async (req, res) => {
  ...
  const existing = await db.collection<ContractTemplateDoc>('contract_templates').findOne({ key: body.key })
  if (existing) {
    return res.status(409).json({ data: null, error: 'duplicate_key' })
  }

  const doc: ContractTemplateDoc = {
    _id: new ObjectId(),
    key: body.key,
    name: body.name,
  ...
  await db.collection<ContractTemplateDoc>('contract_templates').insertOne(doc)
...
contractTemplatesRouter.put('/:id', async (req, res) => {
  ...
  const body = parsed.data
  const update: Partial<ContractTemplateDoc> = {}
  if (body.key !== undefined) update.key = body.key
  if (body.name !== undefined) update.name = body.name
  if (body.description !== undefined) update.description = body.description
  if (body.htmlBody !== undefined) update.htmlBody = body.htmlBody
  update.updatedAt = new Date()

  const coll = db.collection<ContractTemplateDoc>('contract_templates')
