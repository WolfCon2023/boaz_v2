...
webhooksRouter.post('/stripe', async (req, res) => {
  ...
  try {
    ...
    const signatures = sig.split(',').reduce((acc: any, pair) => {
 ...
 */
async function handleStripeCheckoutCompleted(db: any, session: any) {
 ...
 */
async function handleStripePaymentSucceeded(db: any, paymentIntent: any) {
 ...
 */
async function handleStripePaymentFailed(db: any, paymentIntent: any) {
 ...
 */
async function handleStripeRefund(db: any, charge: any) {
 ...
 */
async function handlePayPalPaymentCompleted(db: any, capture: any) {
 ...
 */
async function handlePayPalPaymentFailed(db: any, capture: any) {
 ...
 */
async function handlePayPalRefund(db: any, refund: any) {
  console.log(`[PayPal] Processing refund: ${refund.id}`)

  // Find the original payment
  const payment = await db.collection('payments').findOne({
    paypalTransactionId: refund.links?.find((l: any) => l.rel === 'up')?.href?.split('/').pop()
  })
 ...
 */
async function recordPaymentFromWebhook(db: any, data: {
  invoiceId: string
  amount: number
  method: string
  reference: string
