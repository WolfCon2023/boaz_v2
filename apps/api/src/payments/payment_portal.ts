...
paymentPortalRouter.post('/record', async (req, res) => {
  ...
  try {
    const auth = (req as any).auth as { userId: string; email: string; name?: string }
    const { 
      invoiceId, 
      invoiceNumber,
      amount, 
...
paymentPortalRouter.get('/history', async (req, res) => {
  ...
  try {
    ...
    if (dateFrom || dateTo) {
      query.paidAt = {}
      if (dateFrom) {
        query.paidAt.$gte = new Date(dateFrom as string)
      }
      if (dateTo) {
        query.paidAt.$lte = new Date(dateTo as string)
      }
    }
...
paymentPortalRouter.get('/stats', async (req, res) => {
  ...
  try {
    ...
    const totalAmount = recentPayments.reduce((sum: number, p: any) => sum + (p.amount ?? 0), 0)
