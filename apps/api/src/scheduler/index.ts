...
type AvailabilityDay = { day: number; enabled: boolean; startMin: number; endMin: number }
...
type AppointmentStatus = 'booked' | 'cancelled'
type AppointmentDoc = {
  _id: ObjectId
...
function safeMin(v: any, fallback: number) {
  const n = Number(v)
...
async function ensureDefaultAvailability(db: any, ownerUserId: string) {
...
async function ensureContactForAttendee(
  db: any,
  attendee: { firstName?: string | null; lastName?: string | null; name?: string | null; email: string; phone?: string | null },
) {
  try {
    const email = normEmail(attendee.email)
    if (!email) return { contactId: null as string | null }

    const rx = new RegExp(`^${escapeRegex(email)}$`, 'i')
    const existing = await db.collection('contacts').findOne({ email: rx } as any)
    if (existing?._id) return { contactId: String(existing._id) }

    const fullName =
      `${String(attendee.firstName || '').trim()} ${String(attendee.lastName || '').trim()}`.trim() ||
    ...
    return { contactId: null as string | null }
  }
}
...
internal.get('/users', requirePermission('users.read'), async (_req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  const users = await db
    .collection('users')
    .find({} as any)
    .project({ email: 1, name: 1 } as any)
    .sort({ name: 1, email: 1 } as any)
    .limit(500)
    .toArray()
  res.json({
    data: {
      items: users.map((u: any) => ({ id: String(u._id), name: u.name ?? null, email: u.email ?? null })),
    },
    error: null,
  })
})
...
internal.post('/appointment-types', async (req: any, res) => {
  ...
  if (!slug) return res.status(400).json({ data: null, error: 'invalid_slug' })

  // ensure unique per owner
  const existing = await db.collection('scheduler_appointment_types').findOne({ ownerUserId: auth.userId, slug } as any)
  if (existing) return res.status(409).json({ data: null, error: 'slug_taken' })
  ...
  await db.collection('scheduler_appointment_types').insertOne(doc as any)
  res.status(201).json({ data: { _id: String(doc._id) }, error: null })
})

// PUT /api/scheduler/appointment-types/:id
internal.put('/appointment-types/:id', async (req: any, res) => {
  ...
  if (parsed.data.locationType !== undefined) update.locationType = parsed.data.locationType
  if (parsed.data.locationDetails !== undefined) update.locationDetails = parsed.data.locationDetails ?? null
  ...
  if (update.slug) {
    const clash = await db
      .collection('scheduler_appointment_types')
      .findOne({ ownerUserId: auth.userId, slug: update.slug, _id: { $ne: _id } } as any)
    if (clash) return res.status(409).json({ data: null, error: 'slug_taken' })
  }

  const r = await db.collection('scheduler_appointment_types').updateOne({ _id, ownerUserId: auth.userId } as any, { $set: update } as any)
  if (!r.matchedCount) return res.status(404).json({ data: null, error: 'not_found' })
  res.json({ data: { ok: true }, error: null })
})

// DELETE /api/scheduler/appointment-types/:id
internal.delete('/appointment-types/:id', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  const auth = req.auth as { userId: string; email: string }
  let _id: ObjectId
  ...
  const r = await db.collection('scheduler_appointment_types').deleteOne({ _id, ownerUserId: auth.userId } as any)
  if (!r.deletedCount) return res.status(404).json({ data: null, error: 'not_found' })
  res.json({ data: { ok: true }, error: null })
})

// GET /api/scheduler/availability/me
internal.get('/availability/me', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  const auth = req.auth as { userId: string; email: string }
...
internal.put('/availability/me', async (req: any, res) => {
  ...
  await db.collection('scheduler_availability').updateOne(
    { ownerUserId: auth.userId } as any,
    {
      $set: {
...
internal.get('/appointments', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  const auth = req.auth as { userId: string; email: string }
  ...
  const items = await db
    ...
    .find({ ownerUserId: auth.userId, startsAt: { $gte: from, $lt: to } } as any)
    .sort({ startsAt: 1 })
    .toArray()
...
internal.post('/appointments/book', async (req: any, res) => {
  ...
  const type = (await db.collection('scheduler_appointment_types').findOne({ _id: typeId, ownerUserId: auth.userId, active: true } as any)) as any
  if (!type) return res.status(404).json({ data: null, error: 'appointment_type_not_found' })
  ...
  if (startUtc < now || startUtc > max) return res.status(400).json({ data: null, error: 'startsAt_out_of_range' })
  ...
  let scheduledByEmail: string | null = auth.email ?? null
  if (parsed.data.scheduledByUserId && ObjectId.isValid(parsed.data.scheduledByUserId)) {
    try {
      const u = await db.collection('users').findOne({ _id: new ObjectId(parsed.data.scheduledByUserId) } as any)
      if (u?._id) {
...
internal.post('/appointments/:id/cancel', async (req: any, res) => {
  ...
  db.collection('crm_tasks')
    .updateMany(
      { 'metadata.source': 'scheduler', 'metadata.appointmentId': String(_id) } as any,
...
publicRouter.post('/book/:slug', async (req, res) => {
  ...
  dispatchCrmEvent(
    ...
    { source: 'scheduler_public_booking' },
  ).catch(() => null)
