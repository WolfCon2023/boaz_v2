...
type AccountStatus = 'active' | 'disconnected' | 'expired' | 'error'
...
socialAccountsRouter.post('/accounts', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const now = new Date()
    const doc: Omit<SocialAccountDoc, '_id'> = {
      platform: parsed.platform,
...
socialAccountsRouter.put('/accounts/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (req.body.accessToken) updates.accessToken = req.body.accessToken
    if (req.body.refreshToken) updates.refreshToken = req.body.refreshToken
    if (req.body.followerCount !== undefined) updates.followerCount = req.body.followerCount
    if (req.body.lastSync) updates.lastSync = new Date(req.body.lastSync)
