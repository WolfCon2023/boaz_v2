...
type SocialPostDoc = {
  ...
  accountIds: string[] // Map to social_accounts._id
  images?: string[]
  videoUrl?: string
  link?: string
  linkTitle?: string
  ...
  platformPostIds?: {
    [platform: string]: string // Platform-specific post ID
  }
  error?: string
}
...
socialPostsRouter.post('/posts', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const doc: Omit<SocialPostDoc, '_id'> = {
...
socialPostsRouter.put('/posts/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (req.body.accountIds) updates.accountIds = req.body.accountIds
    ...
    if (req.body.linkTitle !== undefined) updates.linkTitle = req.body.linkTitle
    if (req.body.linkDescription !== undefined) updates.linkDescription = req.body.linkDescription
    ...
    if (req.body.platformPostIds) updates.platformPostIds = req.body.platformPostIds
...
socialPostsRouter.post('/posts/:id/publish', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const post = await db.collection('social_posts').findOne({ _id }) as SocialPostDoc | null
    if (!post) {
      return res.status(404).json({ data: null, error: 'post_not_found' })
    }
...
socialPostsRouter.get('/analytics', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (startDate || endDate) {
      match.publishedAt = {}
      if (startDate) match.publishedAt.$gte = new Date(startDate)
      if (endDate) {
        const end = new Date(endDate)
        end.setHours(23, 59, 59, 999)
    ...
    posts.forEach((post: any) => {
      if (post.metrics) {
        Object.entries(post.metrics).forEach(([platform, metrics]: [string, any]) => {
