 ...
 * Publish to Facebook
 */
async function publishToFacebook(account: any, post: any): Promise<{ success: boolean; platformPostId?: string; error?: string }> {
  try {
    ...
    if (post.link) {
      ...
      if (post.linkTitle) payload.name = post.linkTitle
      if (post.linkDescription) payload.description = post.linkDescription
    }
    ...
    const response = await fetch(
      `https://graph.facebook.com/v18.0/${accountId}/feed?access_token=${accessToken}`,
      {
        method: 'POST',
 ...
 * Publish to Twitter
 */
async function publishToTwitter(account: any, post: any): Promise<{ success: boolean; platformPostId?: string; error?: string }> {
  try {
 ...
 * Publish to LinkedIn
 */
async function publishToLinkedIn(account: any, post: any): Promise<{ success: boolean; platformPostId?: string; error?: string }> {
  try {
    const { accessToken, accountId } = account
    if (!accessToken) {
...
socialPublishRouter.post('/publish/:postId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const post = await db.collection('social_posts').findOne({ _id: postId }) as any
    ...
    const accountIds = (post.accountIds || []).map((id: string) => new ObjectId(id))
