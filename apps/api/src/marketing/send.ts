...
function injectRecipientIntoTrackingLinks(html: string, baseUrl: string, email: string): string {
...
async function wrapLinksWithTracking(
  ...
  for (const match of matches) {
    ...
    if (originalUrl.includes('/api/marketing/r/')) continue
    
    // Skip unsubscribe and pixel tracking URLs
    if (
      originalUrl.includes('/api/marketing/unsubscribe') ||
...
marketingSendRouter.post('/campaigns/:id/send', async (req, res) => {
  ...
  try {
    ...
    for (const r of finalList) {
      ...
      // Use the API's own URL (from request) for unsubscribe links - NOT env.ORIGIN which is the frontend
