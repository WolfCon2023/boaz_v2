...
function customerTicketOwnershipQuery(customerAuth: any) {
  const { accountId, email } = customerAuth || {}
  if (accountId) return { accountId: new ObjectId(accountId) }
  if (email) return { requesterEmail: email }
  return null
}
...
customerPortalDataRouter.post('/tickets/:id/attachments', uploadTicketAttachments.array('files', 10), async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  try {
    const { customerId, name, email } = req.customerAuth || {}
    const ticketId = req.params.id
    if (!ObjectId.isValid(ticketId)) return res.status(400).json({ data: null, error: 'invalid_id' })
    ...
    await db.collection('support_tickets').updateOne(
      { _id },
      { $push: { attachments: { $each: attachments } }, $set: { updatedAt: new Date() } } as any,
    )
...
customerPortalDataRouter.get('/tickets/:id/attachments/:attachmentId/download', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  try {
    const ticketId = req.params.id
