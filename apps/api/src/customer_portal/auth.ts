...
type CustomerPortalUser = {
  ...
  company?: string
  phone?: string
  accountId?: ObjectId // Link to CRM account
...
export function verifyCustomerToken(req: any, res: any, next: any) {
...
customerPortalAuthRouter.post('/register', async (req, res) => {
  ...
  try {
    ...
    const newCustomer: CustomerPortalUser = {
      email: body.email.toLowerCase(),
      passwordHash,
      name: body.name,
...
customerPortalAuthRouter.get('/me', verifyCustomerToken, async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
