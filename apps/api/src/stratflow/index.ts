...
type AutomationRuleDoc = {
  ...
  trigger: {
    ...
    toStatusKey?: IssueStatusKey | null
    linkType?: IssueLinkType | null
  }
  conditions?: {
    issueType?: IssueType | null
    hasLabel?: string | null
    notHasLabel?: string | null
    isBlocked?: boolean | null
  } | null
  actions: {
    addLabels?: string[]
    removeLabels?: string[]
    // Sprint close actions
    moveOpenIssuesToBacklog?: boolean
  }
  createdAt: Date
  updatedAt: Date
}
...
function isIssueBlocked(issue: any) {
  const links = (issue?.links || []) as Array<{ type: string }>
  return links.some((l) => String(l?.type || '') === 'blocked_by')
}

function canEditAutomation(project: any, authUserId: string) {
  return String(project?.ownerId || '') === String(authUserId || '')
}

async function applyAutomationForIssue(
  db: any,
  project: any,
  authUserId: string,
  trigger: { kind: AutomationTriggerKind; toStatusKey?: IssueStatusKey | null; linkType?: IssueLinkType | null },
  issue: any,
) {
  const rules = await db
    .collection('sf_automation_rules')
    .find({ projectId: project._id, enabled: true, 'trigger.kind': trigger.kind } as any)
  ...
  for (const r of rules as any[]) {
    ...
    if (c.hasLabel) {
      const labels = Array.isArray(issue.labels) ? issue.labels.map((x: any) => String(x || '').trim()) : []
      if (!labels.includes(String(c.hasLabel))) continue
    }
    if (c.notHasLabel) {
      const labels = Array.isArray(issue.labels) ? issue.labels.map((x: any) => String(x || '').trim()) : []
      if (labels.includes(String(c.notHasLabel))) continue
    }
    if (c.isBlocked != null) {
      const blocked = isIssueBlocked(issue)
      if (Boolean(c.isBlocked) !== blocked) continue
    }
    ...
    if (addLabels.length) updateDoc.$addToSet = { labels: { $each: addLabels } }
    if (removeLabels.length) updateDoc.$pull = { labels: { $in: removeLabels } }
    if (!addLabels.length && !removeLabels.length) continue

    await db.collection('sf_issues').updateOne({ _id: issue._id } as any, updateDoc as any)
    applied++
...
async function applyAutomationForSprintClose(db: any, project: any, authUserId: string, sprintId: ObjectId) {
  const rules = await db
    .collection('sf_automation_rules')
    .find({ projectId: project._id, enabled: true, 'trigger.kind': 'sprint_closed' } as any)
    .sort({ updatedAt: -1 } as any)
    .limit(200)
    .toArray()
  if (!rules.length) return { applied: 0 }
...
function candidateMentionHandlesForUser(u: { id: string; email?: string; name?: string }) {
...
function extractMentionedUserIds(body: string, users: Array<{ id: string; email?: string; name?: string }>) {
...
async function notifyFromActivity(db: any, activity: Omit<ActivityDoc, '_id'>) {
  try {
    const projectId = activity.projectId as ObjectId
    const issueId = activity.issueId ? (activity.issueId as ObjectId) : null
    const sprintId = activity.sprintId ? (activity.sprintId as ObjectId) : null
    const actorId = String(activity.actorId || '').trim()
    const meta = activity.meta || {}

    const watcherQuery: any = { projectId }
    if (issueId) watcherQuery.$or = [{ issueId: null }, { issueId }]
    else watcherQuery.issueId = null
    const watchRows = (await db.collection('sf_watches').find(watcherQuery).project({ userId: 1 }).toArray()) as Array<{ userId: string }>
    const watchUserIds = watchRows.map((r) => String(r.userId || '').trim()).filter(Boolean)

    const mentionUserIds: string[] = Array.isArray(meta.mentions)
      ? meta.mentions.map((x: any) => String(x || '').trim()).filter(Boolean)
      : []
    const extraDirectUserIds: string[] = Array.isArray(meta.notifyUserIds)
      ? meta.notifyUserIds.map((x: any) => String(x || '').trim()).filter(Boolean)
      : []

    const recipients = Array.from(new Set([...watchUserIds, ...mentionUserIds, ...extraDirectUserIds])).filter((uid) => uid && uid !== actorId)
    if (!recipients.length) return

    const actor = ObjectId.isValid(actorId) ? await db.collection('users').findOne({ _id: new ObjectId(actorId) } as any) : null
    const actorLabel = String((actor as any)?.name || (actor as any)?.email || actorId || 'Someone')

    const issue = issueId ? await db.collection('sf_issues').findOne({ _id: issueId } as any) : null
    const issueLabel = issue ? `${normIssueType((issue as any).type)}: ${String((issue as any).title || '').trim() || 'Untitled'}` : issueId ? `Issue ${String(issueId)}` : null

    const sprint = sprintId ? await db.collection('sf_sprints').findOne({ _id: sprintId } as any) : null
    const sprintLabel = sprint ? String((sprint as any).name || '').trim() : sprintId ? `Sprint ${String(sprintId)}` : null

    const kindMap: Record<ActivityKind, NotificationKind> = {
    ...
    const notifKind: NotificationKind = kindMap[activity.kind] || 'issue_updated'
    ...
    const mentionedOnly = mentionUserIds.filter((uid: string) => uid && uid !== actorId)
    const mentionDocs: NotificationDoc[] = mentionedOnly.length
      ? mentionedOnly.map((userId: string) => ({
          _id: new ObjectId(),
          projectId,
          userId,
          kind: 'mentioned',
    ...
    await db.collection('sf_notifications').insertMany([...docs, ...mentionDocs] as any, { ordered: false } as any).catch(() => {})
  } catch {
    // best-effort
  }
}

async function logActivity(db: any, doc: Omit<ActivityDoc, '_id'>) {
  try {
    await db.collection('sf_activity').insertOne({ _id: new ObjectId(), ...doc } as any)
...
function projectMemberIds(project: any): string[] {
  const ids: string[] = []
  if (project?.ownerId) ids.push(String(project.ownerId))
...
async function loadUsersByIds(db: any, userIds: string[]): Promise<Array<{ id: string; email: string; name: string }>> {
  const valid = userIds.filter((id) => ObjectId.isValid(id))
  const oids = valid.map((id) => new ObjectId(id))
  const users = await db.collection('users').find({ _id: { $in: oids } } as any).project({ _id: 1, email: 1, name: 1 }).toArray()
  const map = new Map(users.map((u: any) => [String(u._id), { id: String(u._id), email: u.email || '', name: u.name || '' }]))
  return valid.map((id) => map.get(id)).filter((x): x is { id: string; email: string; name: string } => Boolean(x))
}

async function validateComponentsExist(db: any, projectId: ObjectId, components: string[]) {
  const names = Array.from(new Set((components || []).map((x) => String(x || '').trim()).filter(Boolean)))
  if (!names.length) return true
  const lowers = names.map((n) => n.toLowerCase())
  const rows = await db.collection('sf_components').find({ projectId, nameLower: { $in: lowers } } as any).project({ nameLower: 1 }).toArray()
  const ok = new Set(rows.map((r: any) => String(r.nameLower)))
...
function parseDateOnlyOrIsoToUtcMidnight(input: any): Date | null {
  const raw = String(input || '').trim()
  if (!raw) return null
  // date-only
...
stratflowRouter.patch('/columns/:columnId', async (req: any, res) => {
  ...
  if (!cid) return res.status(400).json({ data: null, error: 'invalid_column_id' })

  const col = await db.collection('sf_columns').findOne({ _id: cid } as any)
  if (!col) return res.status(404).json({ data: null, error: 'not_found' })
  const board = await db.collection('sf_boards').findOne({ _id: col.boardId } as any)
  ...
  await db
    ...
    .updateOne({ _id: cid } as any, { $set: { wipLimit: parsed.data.wipLimit, updatedAt: new Date() } } as any)
...
stratflowRouter.post('/boards/:boardId/issues', async (req: any, res) => {
  ...
  if (wipLimit != null) {
    const curCount = await db.collection('sf_issues').countDocuments({ boardId: bid, columnId: colId } as any)
    if (curCount >= wipLimit) return res.status(400).json({ data: null, error: 'wip_limit_reached' })
  }
  ...
  if (doc.assigneeId) {
    ...
    if (!allowed.includes(doc.assigneeId)) return res.status(400).json({ data: null, error: 'invalid_assignee' })
  }
...
stratflowRouter.patch('/issues/:issueId/move', async (req: any, res) => {
  ...
  const nextIssue = await db.collection('sf_issues').findOne({ _id: iid } as any)
  if (nextIssue) {
    await applyAutomationForIssue(
      db,
      project,
      auth.userId,
      { kind: 'issue_moved', toStatusKey: nextStatusKey },
      nextIssue,
    )
  }
...
stratflowRouter.patch('/sprints/:sprintId', async (req: any, res) => {
  ...
  if (parsed.data.capacityPoints !== undefined) update.capacityPoints = parsed.data.capacityPoints ?? null
...
stratflowRouter.post('/sprints/:sprintId/close', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  if (String(project.ownerId) !== String(auth.userId)) return res.status(403).json({ data: null, error: 'owner_only' })

  if (!force) {
    const openCount = await db.collection('sf_issues').countDocuments({ projectId: sprint.projectId, sprintId: sid, statusKey: { $ne: 'done' } } as any)
    if (openCount > 0) return res.status(400).json({ data: null, error: 'sprint_has_open_work', openCount })
  }

  const now = new Date()
  await db.collection('sf_sprints').updateOne({ _id: sid } as any, { $set: { state: 'closed', updatedAt: now } } as any)
...
stratflowRouter.post('/issues/:issueId/links', async (req: any, res) => {
  ...
  if (!oid) return res.status(400).json({ data: null, error: 'invalid_other_issue_id' })
  if (String(iid) === String(oid)) return res.status(400).json({ data: null, error: 'cannot_link_self' })
  ...
  const other = await db.collection('sf_issues').findOne({ _id: oid, projectId: issue.projectId } as any)
  if (!other) return res.status(400).json({ data: null, error: 'invalid_other_issue' })

  await db.collection('sf_issues').updateOne(
    { _id: iid } as any,
    { $addToSet: { links: { type: parsed.data.type, issueId: oid } }, $set: { updatedAt: new Date() } } as any,
  )
  ...
  if (nextIssue) {
    await applyAutomationForIssue(
      ...
      project,
      auth.userId,
      { kind: 'issue_link_added', linkType: parsed.data.type as any },
      nextIssue,
    )
  }
  await logActivity(db, {
...
stratflowRouter.post('/issues/:issueId/links/remove', async (req: any, res) => {
  const parsed = issueLinkSchema.safeParse(req.body ?? {})
  if (!parsed.success) return res.status(400).json({ data: null, error: 'invalid_payload', details: parsed.error.flatten() })
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  ...
  await db.collection('sf_issues').updateOne(
    { _id: iid } as any,
    { $pull: { links: { type: parsed.data.type, issueId: oid } }, $set: { updatedAt: new Date() } } as any,
  )
  ...
  if (nextIssue) {
    await applyAutomationForIssue(
      ...
      project,
      auth.userId,
      { kind: 'issue_link_removed', linkType: parsed.data.type as any },
      nextIssue,
    )
  }
  await logActivity(db, {
...
stratflowRouter.patch('/issues/:issueId', async (req: any, res) => {
  ...
  if (parsed.data.components !== undefined) {
    ...
    const ok = await validateComponentsExist(db, issue.projectId as any, next)
    if (!ok) return res.status(400).json({ data: null, error: 'invalid_components' })
    update.components = next
  }
  if (parsed.data.phase !== undefined) update.phase = parsed.data.phase ? String(parsed.data.phase).trim() : null
  if (parsed.data.targetStartDate !== undefined) update.targetStartDate = parsed.data.targetStartDate ? new Date(parsed.data.targetStartDate) : null
  if (parsed.data.targetEndDate !== undefined) update.targetEndDate = parsed.data.targetEndDate ? new Date(parsed.data.targetEndDate) : null
...
stratflowRouter.get('/issues/:issueId/time-entries', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const items = await db
    ...
    .find({ issueId: iid } as any)
    .sort({ workDate: -1, createdAt: -1 } as any)
    .limit(2000)
    .toArray()
...
stratflowRouter.post('/issues/:issueId/time-entries', async (req: any, res) => {
  ...
  const iid = objIdOrNull(String(req.params.issueId || ''))
  if (!iid) return res.status(400).json({ data: null, error: 'invalid_issue_id' })
  if (!isValidUserId(auth.userId)) return res.status(400).json({ data: null, error: 'invalid_user_id' })
  ...
  const allowed = projectMemberIds(project)
  if (!allowed.includes(String(auth.userId))) return res.status(403).json({ data: null, error: 'forbidden' })
  ...
  await db.collection('sf_time_entries').insertOne(doc as any)
  await logActivity(db, {
    projectId: issue.projectId,
    actorId: auth.userId,
    kind: 'time_logged',
...
stratflowRouter.patch('/time-entries/:timeEntryId', async (req: any, res) => {
  ...
  if (!tid) return res.status(400).json({ data: null, error: 'invalid_time_entry_id' })

  const existing = await db.collection('sf_time_entries').findOne({ _id: tid } as any)
  if (!existing) return res.status(404).json({ data: null, error: 'not_found' })
  const project = await loadProjectForUser(db, existing.projectId as any, auth.userId)
  if (project === 'forbidden') return res.status(403).json({ data: null, error: 'forbidden' })
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })
  ...
  const update: any = { updatedAt: new Date() }
  if (parsed.data.minutes !== undefined) update.minutes = parsed.data.minutes
  if (parsed.data.billable !== undefined) update.billable = Boolean(parsed.data.billable)
  if (parsed.data.note !== undefined) update.note = parsed.data.note ? String(parsed.data.note).trim() : null
  if (parsed.data.workDate !== undefined) {
    ...
    if (!update.workDate) return res.status(400).json({ data: null, error: 'invalid_work_date' })
  }

  await db.collection('sf_time_entries').updateOne({ _id: tid } as any, { $set: update } as any)
  await logActivity(db, {
...
stratflowRouter.delete('/time-entries/:timeEntryId', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  await db.collection('sf_time_entries').deleteOne({ _id: tid } as any)
  await logActivity(db, {
    projectId: existing.projectId,
    actorId: auth.userId,
    kind: 'time_deleted',
...
stratflowRouter.get('/projects/:projectId/members', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
...
stratflowRouter.get('/projects/:projectId/watches/me', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const rows = await db.collection('sf_watches').find({ projectId: pid, userId: auth.userId } as any).toArray()
  const projectWatch = rows.some((r: any) => !r.issueId)
  const issueIds = rows.filter((r: any) => r.issueId).map((r: any) => String(r.issueId))
...
stratflowRouter.post('/projects/:projectId/watch', async (req: any, res) => {
  ...
  if (!enabled) {
    await db.collection('sf_watches').deleteOne({ projectId: pid, userId: auth.userId, issueId: null } as any)
  } else {
    const now = new Date()
    await db.collection('sf_watches').updateOne(
      { projectId: pid, userId: auth.userId, issueId: null } as any,
      { $setOnInsert: { _id: new ObjectId(), projectId: pid, userId: auth.userId, issueId: null, createdAt: now } } as any,
      { upsert: true } as any,
    )
  }
...
stratflowRouter.post('/issues/:issueId/watch', async (req: any, res) => {
  const parsed = watchToggleSchema.safeParse(req.body ?? {})
  if (!parsed.success) return res.status(400).json({ data: null, error: 'invalid_payload', details: parsed.error.flatten() })
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  ...
  const enabled = parsed.data.enabled !== false
  if (!enabled) {
    await db.collection('sf_watches').deleteOne({ projectId: issue.projectId, userId: auth.userId, issueId: iid } as any)
  } else {
    const now = new Date()
    await db.collection('sf_watches').updateOne(
      { projectId: issue.projectId, userId: auth.userId, issueId: iid } as any,
      { $setOnInsert: { _id: new ObjectId(), projectId: issue.projectId, userId: auth.userId, issueId: iid, createdAt: now } } as any,
      { upsert: true } as any,
    )
  }
  res.json({ data: { ok: true, enabled }, error: null })
...
stratflowRouter.get('/projects/:projectId/notifications', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const query: any = { projectId: pid, userId: auth.userId }
  if (unreadOnly) query.readAt = null

  const items = await db.collection('sf_notifications').find(query).sort({ createdAt: -1 } as any).limit(limit).toArray()
  res.json({
    data: {
      projectId: String(pid),
...
stratflowRouter.post('/notifications/:notificationId/read', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  const nid = objIdOrNull(String(req.params.notificationId || ''))
  if (!nid) return res.status(400).json({ data: null, error: 'invalid_notification_id' })

  const existing = await db.collection('sf_notifications').findOne({ _id: nid } as any)
  if (!existing) return res.status(404).json({ data: null, error: 'not_found' })
  if (String(existing.userId) !== String(auth.userId)) return res.status(403).json({ data: null, error: 'forbidden' })

  const project = await loadProjectForUser(db, existing.projectId as any, auth.userId)
  if (project === 'forbidden') return res.status(403).json({ data: null, error: 'forbidden' })
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })

  await db.collection('sf_notifications').updateOne({ _id: nid } as any, { $set: { readAt: new Date() } } as any)
  res.json({ data: { ok: true }, error: null })
})

// POST /api/stratflow/projects/:projectId/notifications/mark-all-read
stratflowRouter.post('/projects/:projectId/notifications/mark-all-read', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const now = new Date()
  const r = await db.collection('sf_notifications').updateMany({ projectId: pid, userId: auth.userId, readAt: null } as any, { $set: { readAt: now } } as any)
...
stratflowRouter.get('/projects/:projectId/automation-rules', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const items = await db.collection('sf_automation_rules').find({ projectId: pid } as any).sort({ updatedAt: -1 } as any).limit(200).toArray()
  res.json({
    data: {
      projectId: String(pid),
...
stratflowRouter.post('/projects/:projectId/automation-rules', async (req: any, res) => {
  ...
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })
  if (project === 'forbidden') return res.status(403).json({ data: null, error: 'forbidden' })
  if (!canEditAutomation(project, auth.userId)) return res.status(403).json({ data: null, error: 'owner_only' })

  const now = new Date()
  const doc: AutomationRuleDoc = {
    _id: new ObjectId(),
    projectId: pid,
    name: parsed.data.name.trim(),
    enabled: parsed.data.enabled !== false,
    trigger: {
      kind: parsed.data.trigger.kind as any,
  ...
  await db.collection('sf_automation_rules').insertOne(doc as any)
  res.status(201).json({ data: { _id: String(doc._id) }, error: null })
})

// PATCH /api/stratflow/automation-rules/:ruleId (owner-only)
stratflowRouter.patch('/automation-rules/:ruleId', async (req: any, res) => {
  ...
  if (!rid) return res.status(400).json({ data: null, error: 'invalid_rule_id' })

  const existing = await db.collection('sf_automation_rules').findOne({ _id: rid } as any)
  if (!existing) return res.status(404).json({ data: null, error: 'not_found' })
  const project = await loadProjectForUser(db, existing.projectId as any, auth.userId)
  if (project === 'forbidden') return res.status(403).json({ data: null, error: 'forbidden' })
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })
  ...
  const update: any = { updatedAt: new Date() }
  if (parsed.data.name !== undefined) update.name = String(parsed.data.name).trim()
  if (parsed.data.enabled !== undefined) update.enabled = Boolean(parsed.data.enabled)
  if (parsed.data.trigger !== undefined) update.trigger = { ...(existing.trigger || {}), ...(parsed.data.trigger as any) }
  if (parsed.data.conditions !== undefined) update.conditions = parsed.data.conditions ?? null
  if (parsed.data.actions !== undefined) {
    update.actions = {
      ...(existing.actions || {}),
      ...(parsed.data.actions as any),
    }
    if (update.actions.addLabels !== undefined) update.actions.addLabels = normStrArray(update.actions.addLabels, 50)
    if (update.actions.removeLabels !== undefined) update.actions.removeLabels = normStrArray(update.actions.removeLabels, 50)
    if (update.actions.moveOpenIssuesToBacklog !== undefined) update.actions.moveOpenIssuesToBacklog = Boolean(update.actions.moveOpenIssuesToBacklog)
  }

  await db.collection('sf_automation_rules').updateOne({ _id: rid } as any, { $set: update } as any)
  res.json({ data: { ok: true }, error: null })
})

// DELETE /api/stratflow/automation-rules/:ruleId (owner-only)
stratflowRouter.delete('/automation-rules/:ruleId', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  await db.collection('sf_automation_rules').deleteOne({ _id: rid } as any)
  res.json({ data: { ok: true }, error: null })
})

// GET /api/stratflow/projects/:projectId/time-rollups
stratflowRouter.get('/projects/:projectId/time-rollups', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const totalsRow = (
    await db
      ...
      .toArray()
  )[0] || { totalMinutes: 0, billableMinutes: 0, entryCount: 0 }
...
stratflowRouter.get('/projects/:projectId/time-entries', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const match: any = { projectId: pid }
  if (issueIdQ) match.issueId = issueIdQ
  if (userIdQ) match.userId = userIdQ
  if (billableQ === 'true') match.billable = true
  if (billableQ === 'false') match.billable = false
  if (from || to) {
    const range: any = {}
    if (from) range.$gte = from
    if (to) range.$lt = new Date(to.getTime() + 24 * 60 * 60 * 1000)
  ...
  const issueIds = Array.from(new Set(items.map((it: any) => it.issueId).filter(Boolean))) as ObjectId[]
  const userIds = Array.from(new Set(items.map((it: any) => String(it.userId || '')).filter((x: string) => ObjectId.isValid(x))))

  const issues = issueIds.length
    ? await db.collection('sf_issues').find({ _id: { $in: issueIds }, projectId: pid } as any).project({ _id: 1, title: 1, type: 1 }).toArray()
    : []
  const issueMap = new Map(issues.map((i: any) => [String(i._id), { title: String(i.title || ''), type: String(i.type || '') }]))
...
stratflowRouter.get('/projects/:projectId/activity', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
...
stratflowRouter.get('/projects/:projectId/epic-rollups', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const epicDocs = await db.collection('sf_issues').find({ projectId: pid, type: 'Epic' } as any).sort({ updatedAt: -1 } as any).limit(2000).toArray()
  const epicIds = epicDocs.map((e: any) => e._id)
  ...
  const byEpicId = new Map<string, any>()
  rows.forEach((r: any) => byEpicId.set(String(r._id), r))

  res.json({
    data: {
      items: epicDocs.map((e: any) => {
...
stratflowRouter.get('/projects/:projectId/components', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const auth = req.auth as { userId: string; email: string }
  ...
  const items = await db.collection('sf_components').find({ projectId: pid } as any).sort({ nameLower: 1 } as any).toArray()
  res.json({
    data: {
      items: items.map((d: any) => ({
        _id: String(d._id),
...
stratflowAdminRouter.get('/projects', async (_req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)

  const items = await db.collection('sf_projects').find({} as any).sort({ updatedAt: -1 } as any).limit(500).toArray()
  const ownerIds = Array.from(new Set(items.map((p: any) => String(p.ownerId || '')).filter((x: string) => ObjectId.isValid(x))))
...
async function deleteProjectCascade(db: any, projectId: ObjectId) {
  const boards = await db.collection('sf_boards').find({ projectId } as any).project({ _id: 1 }).toArray()
  const boardIds = boards.map((b: any) => b._id)
  const now = new Date()
  // delete children first
  const issues = await db.collection('sf_issues').deleteMany({ projectId } as any)
  const comments = await db.collection('sf_issue_comments').deleteMany({ projectId } as any)
  const timeEntries = await db.collection('sf_time_entries').deleteMany({ projectId } as any)
  const watches = await db.collection('sf_watches').deleteMany({ projectId } as any)
  const notifications = await db.collection('sf_notifications').deleteMany({ projectId } as any)
  const activity = await db.collection('sf_activity').deleteMany({ projectId } as any)
  const sprints = await db.collection('sf_sprints').deleteMany({ projectId } as any)
  const components = await db.collection('sf_components').deleteMany({ projectId } as any)
  const columns = boardIds.length ? await db.collection('sf_columns').deleteMany({ boardId: { $in: boardIds } } as any) : { deletedCount: 0 }
  const boardsDel = await db.collection('sf_boards').deleteMany({ projectId } as any)
  const projectDel = await db.collection('sf_projects').deleteOne({ _id: projectId } as any)
  return {
    deletedAt: now.toISOString(),
    project: projectDel.deletedCount || 0,
    boards: boardsDel.deletedCount || 0,
    columns: (columns as any).deletedCount || 0,
...
stratflowAdminRouter.delete('/projects/:projectId', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const pid = objIdOrNull(String(req.params.projectId || ''))
  if (!pid) return res.status(400).json({ data: null, error: 'invalid_project_id' })
  const existing = await db.collection('sf_projects').findOne({ _id: pid } as any)
  if (!existing) return res.status(404).json({ data: null, error: 'not_found' })
...
stratflowAdminRouter.get('/projects/:projectId/members', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const pid = objIdOrNull(String(req.params.projectId || ''))
  ...
  const project = await db.collection('sf_projects').findOne({ _id: pid } as any)
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })
  const ids = projectMemberIds(project)
  const users = await loadUsersByIds(db, ids)
...
stratflowAdminRouter.post('/projects/:projectId/members', async (req: any, res) => {
  ...
  if (!ObjectId.isValid(uid)) return res.status(400).json({ data: null, error: 'invalid_user_id' })
  const user = await db.collection('users').findOne({ _id: new ObjectId(uid) } as any)
  if (!user) return res.status(404).json({ data: null, error: 'user_not_found' })
  if (String(project.ownerId) === uid) return res.json({ data: { ok: true }, error: null })

  await db.collection('sf_projects').updateOne({ _id: pid } as any, { $addToSet: { teamIds: uid }, $set: { updatedAt: new Date() } } as any)
  res.json({ data: { ok: true }, error: null })
})

// DELETE /api/stratflow/admin/projects/:projectId/members/:userId
stratflowAdminRouter.delete('/projects/:projectId/members/:userId', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const pid = objIdOrNull(String(req.params.projectId || ''))
  ...
  const project = await db.collection('sf_projects').findOne({ _id: pid } as any)
  if (!project) return res.status(404).json({ data: null, error: 'not_found' })
  if (String(project.ownerId) === uid) return res.status(400).json({ data: null, error: 'cannot_remove_owner' })

  await db.collection('sf_projects').updateOne({ _id: pid } as any, { $pull: { teamIds: uid }, $set: { updatedAt: new Date() } } as any)
  // Best-effort: clear assignees who are no longer in team
  try {
    await db.collection('sf_issues').updateMany({ projectId: pid, assigneeId: uid } as any, { $set: { assigneeId: null, updatedAt: new Date() } } as any)
  } catch {
    // noop
  }
  res.json({ data: { ok: true }, error: null })
...
stratflowAdminRouter.get('/projects/:projectId/components', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const pid = objIdOrNull(String(req.params.projectId || ''))
  ...
  const items = await db.collection('sf_components').find({ projectId: pid } as any).sort({ nameLower: 1 } as any).toArray()
  res.json({ data: { items: items.map((d: any) => ({ _id: String(d._id), name: d.name, createdAt: toIsoOrNull(d.createdAt), updatedAt: toIsoOrNull(d.updatedAt) })) }, error: null })
})

// POST /api/stratflow/admin/projects/:projectId/components
stratflowAdminRouter.post('/projects/:projectId/components', async (req: any, res) => {
  ...
  const doc: ProjectComponentDoc = { _id: new ObjectId(), projectId: pid, name, nameLower: name.toLowerCase(), createdAt: now, updatedAt: now }
  try {
    await db.collection('sf_components').insertOne(doc as any)
  } catch {
...
stratflowAdminRouter.delete('/projects/:projectId/components/:componentId', async (req: any, res) => {
  const db = await getDb()
  if (!db) return res.status(500).json({ data: null, error: 'db_unavailable' })
  await ensureStratflowIndexes(db)
  const pid = objIdOrNull(String(req.params.projectId || ''))
  ...
  if (!cid) return res.status(400).json({ data: null, error: 'invalid_component_id' })

  const row = await db.collection('sf_components').findOne({ _id: cid, projectId: pid } as any)
  if (!row) return res.status(404).json({ data: null, error: 'not_found' })
  await db.collection('sf_components').deleteOne({ _id: cid } as any)

  // Best-effort: strip this component from issues in the project
  try {
    await db.collection('sf_issues').updateMany(
      { projectId: pid, components: row.name } as any,
      { $pull: { components: row.name }, $set: { updatedAt: new Date() } } as any,
    )
  } catch {
    // noop
  }
