...
`,
    ...
    for (const ARTICLE of ARTICLES) {
      const existing = await db.collection('kb_articles').findOne({ slug: ARTICLE.slug })
      const result = existing ? 'updated' : 'created'
      if (existing) await db.collection('kb_articles').updateOne({ slug: ARTICLE.slug }, { $set: { ...ARTICLE, updatedAt: new Date() } })
      else await db.collection('kb_articles').insertOne(ARTICLE as any)
