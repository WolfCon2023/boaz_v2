        ...
        try {
            ...
            if (err && err.code === 11000 && err.keyPattern?.accountNumber) {
                ...
                const maxDocs = await db
                    ...
                    .limit(1)
                    .toArray();
                const maxNum = maxDocs[0]?.accountNumber ?? 998800;
                ...
                if (auth) {
                    const user = await db.collection('users').findOne({ _id: new ObjectId(auth.userId) });
                    await addAccountHistory(db, retryResult.insertedId, 'created', `Account created: ${parsed.data.name}${parsed.data.companyName ? ` (${parsed.data.companyName})` : ''}`, auth.userId, user?.name, auth.email);
                }
                else {
