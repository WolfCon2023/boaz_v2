...
async function addDocumentHistory(db, documentId, eventType, description, userId, userName, userEmail, oldValue, newValue, metadata) {
    try {
...
async function hasPermission(db, document, userId, requiredPermission) {
    // Owner has all permissions
    if (String(document.ownerId) === userId)
        return true;
...
documentsRouter.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const { q, category, tag, relatedTo, relatedId, sort = 'updatedAt', dir = 'desc' } = req.query;
...
documentsRouter.post('/', requireAuth, upload.single('file'), async (req, res) => {
    try {
        ...
        await addDocumentHistory(db, document._id, 'created', `Document "${name}" created`, auth.userId, user?.name, auth.email, undefined, undefined, { version: 1, filename: file.originalname });
        res.status(201).json({
            data: {
                _id: document._id,
                name: document.name,
...
documentsRouter.get('/:id/download/:versionId', requireAuth, async (req, res) => {
    try {
        ...
        await addDocumentHistory(db, _id, 'version_downloaded', `Version ${version.version} downloaded: ${version.originalFilename}`, auth.userId, undefined, auth.email, undefined, undefined, { version: version.version, filename: version.originalFilename });
...
documentsRouter.get('/:id/download', requireAuth, async (req, res) => {
    try {
        ...
        await addDocumentHistory(db, _id, 'version_downloaded', `Latest version (v${latestVersion.version}) downloaded: ${latestVersion.originalFilename}`, auth.userId, undefined, auth.email, undefined, undefined, { version: latestVersion.version, filename: latestVersion.originalFilename });
...
documentsRouter.post('/:id/versions', requireAuth, upload.single('file'), async (req, res) => {
    try {
        ...
        await db.collection('documents').updateOne({ _id }, {
        ...
        await addDocumentHistory(db, _id, 'version_uploaded', `Version ${newVersion.version} uploaded: ${newVersion.originalFilename}`, auth.userId, user?.name, auth.email, undefined, undefined, { version: newVersion.version, filename: newVersion.originalFilename, size: newVersion.size });
...
documentsRouter.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        await db.collection('documents').updateOne({ _id }, { $set: update });
        // Add history entry for update
        if (changes.length > 0) {
            const user = await db.collection('users').findOne({ _id: new ObjectId(auth.userId) });
            await addDocumentHistory(db, _id, changes.length === 1 ? 'field_changed' : 'updated', changes.length === 1
                ? `${changes[0]} changed from "${oldValues[changes[0]]}" to "${newValues[changes[0]]}"`
                : `Document updated: ${changes.join(', ')}`, auth.userId, user?.name, auth.email, oldValues, newValues);
        }
...
documentsRouter.post('/:id/permissions', requireAuth, async (req, res) => {
    try {
        ...
        await db.collection('documents').updateOne({ _id }, { $pull: { permissions: { userId: targetUserId } } });
        // Add new permission
        const permission = {
            userId: targetUserId,
        ...
        await db.collection('documents').updateOne({ _id }, { $push: { permissions: permission } });
        // Add history entry
        const user = await db.collection('users').findOne({ _id: userId });
        await addDocumentHistory(db, _id, isUpdate ? 'permission_updated' : 'permission_added', isUpdate
            ? `Permission updated for ${targetUser?.name || targetUser?.email}: ${existingPermission?.permission} → ${parsed.data.permission}`
            : `Permission granted to ${targetUser?.name || targetUser?.email}: ${parsed.data.permission}`, auth.userId, user?.name, auth.email, isUpdate ? existingPermission?.permission : undefined, parsed.data.permission, { targetUserId: String(targetUserId), targetUserName: targetUser?.name, targetUserEmail: targetUser?.email });
...
documentsRouter.delete('/:id/permissions/:userId', requireAuth, async (req, res) => {
    try {
        ...
        if (removedPermission) {
            const user = await db.collection('users').findOne({ _id: new ObjectId(auth.userId) });
            const targetUser = await db.collection('users').findOne({ _id: targetUserId });
            await addDocumentHistory(db, _id, 'permission_removed', `Permission removed from ${targetUser?.name || targetUser?.email}: ${removedPermission.permission}`, auth.userId, user?.name, auth.email, removedPermission.permission, undefined, { targetUserId: String(targetUserId), targetUserName: targetUser?.name, targetUserEmail: targetUser?.email });
        }
        res.json({ data: { ok: true }, error: null });
    }
    catch (e) {
...
documentsRouter.post('/:id/checkout', requireAuth, async (req, res) => {
    try {
        ...
        // Add history entry
        await addDocumentHistory(db, _id, 'checked_out', `Document checked out by ${user?.name || auth.email}`, auth.userId, user?.name, auth.email);
        res.json({ data: { ok: true }, error: null });
    }
    catch (e) {
        if (e.message?.includes('ObjectId')) {
...
documentsRouter.post('/:id/checkin', requireAuth, async (req, res) => {
    try {
        ...
        // Add history entry
        await addDocumentHistory(db, _id, 'checked_in', `Document checked in by ${user?.name || auth.email}${!document.checkedOutBy.equals(userId) ? ` (was checked out by ${checkedOutByName})` : ''}`, auth.userId, user?.name, auth.email);
        res.json({ data: { ok: true }, error: null });
    }
    catch (e) {
        if (e.message?.includes('ObjectId')) {
...
documentsRouter.delete('/:id', requireAuth, requirePermission('*'), async (req, res) => {
    try {
        ...
        const user = await db.collection('users').findOne({ _id: new ObjectId(auth.userId) });
        await addDocumentHistory(db, _id, 'deleted', `Document "${document.name}" deleted`, auth.userId, user?.name, auth.email);
        // Delete document record
        await db.collection('documents').deleteOne({ _id });
        res.json({ data: { ok: true }, error: null });
    }
    catch (e) {
        if (e.message?.includes('ObjectId')) {
