...
termsReviewRouter.get('/review/:token', async (req, res) => {
    ...
    try {
        ...
        const request = await db.collection('terms_review_requests').findOne({ reviewToken: token });
        if (!request) {
            return res.status(404).json({ data: null, error: 'review_request_not_found' });
        }
        // Get terms content
        const terms = await db.collection('custom_terms').findOne({ _id: request.termsId });
        if (!terms) {
            return res.status(404).json({ data: null, error: 'terms_not_found' });
        }
        // Mark as viewed if still pending
        if (request.status === 'pending') {
            await db.collection('terms_review_requests').updateOne({ _id: request._id }, { $set: { status: 'viewed', viewedAt: new Date(), updatedAt: new Date() } });
        }
        res.json({
            data: {
                request: {
...
termsReviewRouter.post('/review/:token/respond', async (req, res) => {
    ...
    try {
        ...
        // Update request
        const now = new Date();
        await db.collection('terms_review_requests').updateOne({ _id: request._id }, {
            $set: {
