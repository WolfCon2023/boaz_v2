...
invoicesRouter.get('/:id/history', async (req, res) => {
    ...
    try {
        ...
        if (!inv)
            return res.status(404).json({ data: null, error: 'not_found' });
        const createdAt = inv.createdAt || _id.getTimestamp();
        const payments = inv.payments ?? [];
        const refunds = inv.refunds ?? [];
        res.json({ data: { createdAt, payments, refunds, invoice: { title: inv.title, total: inv.total, status: inv.status, invoiceNumber: inv.invoiceNumber, issuedAt: inv.issuedAt, dueDate: inv.dueDate } }, error: null });
    }
    catch {
        res.status(400).json({ data: null, error: 'invalid_id' });
    }
...
invoicesRouter.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (raw.dueDate != null)
            update.dueDate = raw.dueDate ? new Date(raw.dueDate) : null;
