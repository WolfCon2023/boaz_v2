...
export function buildSignedHtml(contract) {
    const ctx = buildContractContext(contract);
    return renderTemplateString(`<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
...
slasRouter.get('/', async (req, res) => {
    ...
    if (status && statusEnum.options.includes(status)) {
        filter.status = status;
    }
    if (type && typeEnum.options.includes(type)) {
        filter.type = type;
    }
...
slasRouter.get('/:id', async (req, res) => {
    ...
    if (!ObjectId.isValid(id))
        return res.status(400).json({ data: null, error: 'invalid_id' });
    const doc = await db.collection('sla_contracts').findOne({ _id: new ObjectId(id) });
    if (!doc)
        return res.status(404).json({ data: null, error: 'not_found' });
...
slasRouter.post('/', async (req, res) => {
    ...
    const now = new Date();
    const contractNumber = typeof body.contractNumber === 'number' && body.contractNumber > 0
        ? body.contractNumber
    ...
    const doc = {
        ...
        // Links
        primaryQuoteId: body.primaryQuoteId && ObjectId.isValid(body.primaryQuoteId)
            ? new ObjectId(body.primaryQuoteId)
            : null,
        primaryDealId: body.primaryDealId && ObjectId.isValid(body.primaryDealId)
        ...
        parentContractId: body.parentContractId && ObjectId.isValid(body.parentContractId)
    ...
    await db.collection('sla_contracts').insertOne(doc);
...
slasRouter.put('/:id', async (req, res) => {
    ...
    if (body.contractNumber !== undefined)
        update.contractNumber = body.contractNumber ?? null;
    ...
    if (body.effectiveDate !== undefined)
        update.effectiveDate = parseDate(body.effectiveDate);
    if (body.startDate !== undefined)
        update.startDate = parseDate(body.startDate);
    if (body.endDate !== undefined)
        update.endDate = parseDate(body.endDate);
    if (body.autoRenew !== undefined)
        update.autoRenew = body.autoRenew;
    if (body.renewalDate !== undefined)
        update.renewalDate = parseDate(body.renewalDate);
    if (body.renewalTermMonths !== undefined)
        update.renewalTermMonths = body.renewalTermMonths ?? null;
    if (body.noticePeriodDays !== undefined)
        update.noticePeriodDays = body.noticePeriodDays ?? null;
    if (body.billingFrequency !== undefined)
        update.billingFrequency = body.billingFrequency;
    if (body.currency !== undefined)
        update.currency = body.currency;
    if (body.baseAmountCents !== undefined)
        update.baseAmountCents = body.baseAmountCents ?? null;
    if (body.invoiceDueDays !== undefined)
        update.invoiceDueDays = body.invoiceDueDays ?? null;
    if (body.uptimeTargetPercent !== undefined)
        update.uptimeTargetPercent = body.uptimeTargetPercent ?? null;
    if (body.supportHours !== undefined)
        update.supportHours = body.supportHours;
    if (body.slaExclusionsSummary !== undefined)
        update.slaExclusionsSummary = body.slaExclusionsSummary;
    if (body.responseTargetMinutes !== undefined)
        update.responseTargetMinutes = body.responseTargetMinutes ?? null;
    if (body.resolutionTargetMinutes !== undefined)
        update.resolutionTargetMinutes = body.resolutionTargetMinutes ?? null;
    ...
    if (body.slaProfiles !== undefined)
        update.slaProfiles = body.slaProfiles;
    // Parties & ownership
    if (body.customerLegalName !== undefined)
        update.customerLegalName = body.customerLegalName;
    if (body.customerAddress !== undefined)
        update.customerAddress = body.customerAddress;
    if (body.customerExecSponsor !== undefined)
        update.customerExecSponsor = body.customerExecSponsor;
    if (body.customerTechContact !== undefined)
        update.customerTechContact = body.customerTechContact;
    if (body.providerLegalName !== undefined)
        update.providerLegalName = body.providerLegalName;
    if (body.providerAddress !== undefined)
        update.providerAddress = body.providerAddress;
    if (body.providerAccountManager !== undefined)
        update.providerAccountManager = body.providerAccountManager;
    if (body.providerCsm !== undefined)
        update.providerCsm = body.providerCsm;
    ...
    if (body.governingLaw !== undefined)
        update.governingLaw = body.governingLaw;
    if (body.jurisdiction !== undefined)
        update.jurisdiction = body.jurisdiction;
    if (body.paymentTerms !== undefined)
        update.paymentTerms = body.paymentTerms;
    if (body.serviceScopeSummary !== undefined)
        update.serviceScopeSummary = body.serviceScopeSummary;
    if (body.limitationOfLiability !== undefined)
        update.limitationOfLiability = body.limitationOfLiability;
    if (body.indemnificationSummary !== undefined)
        update.indemnificationSummary = body.indemnificationSummary;
    if (body.confidentialitySummary !== undefined)
        update.confidentialitySummary = body.confidentialitySummary;
    if (body.dataProtectionSummary !== undefined)
        update.dataProtectionSummary = body.dataProtectionSummary;
    if (body.ipOwnershipSummary !== undefined)
        update.ipOwnershipSummary = body.ipOwnershipSummary;
    if (body.terminationConditions !== undefined)
        update.terminationConditions = body.terminationConditions;
    if (body.changeOrderProcess !== undefined)
        update.changeOrderProcess = body.changeOrderProcess;
    if (body.dataClassification !== undefined)
        update.dataClassification = body.dataClassification;
    if (body.hasDataProcessingAddendum !== undefined)
        update.hasDataProcessingAddendum = body.hasDataProcessingAddendum;
    // Change control & negotiations
    if (body.changeControlRequiredFor !== undefined)
        update.changeControlRequiredFor = body.changeControlRequiredFor;
    if (body.negotiationStatus !== undefined)
        update.negotiationStatus = body.negotiationStatus;
    if (body.redlineSummary !== undefined)
        update.redlineSummary = body.redlineSummary;
    // Audit / rights
    if (body.auditRightsSummary !== undefined)
        update.auditRightsSummary = body.auditRightsSummary;
    if (body.usageRestrictionsSummary !== undefined)
        update.usageRestrictionsSummary = body.usageRestrictionsSummary;
    if (body.subprocessorUseSummary !== undefined)
        update.subprocessorUseSummary = body.subprocessorUseSummary;
    // Renewals / levers
    if (body.autoIncreasePercentOnRenewal !== undefined) {
        update.autoIncreasePercentOnRenewal = body.autoIncreasePercentOnRenewal ?? null;
    }
    if (body.earlyTerminationFeeModel !== undefined)
        update.earlyTerminationFeeModel = body.earlyTerminationFeeModel;
    if (body.upsellCrossSellRights !== undefined)
        update.upsellCrossSellRights = body.upsellCrossSellRights;
    // Links
    ...
    if (body.coveredAssetTags !== undefined)
        update.coveredAssetTags = body.coveredAssetTags ?? [];
    if (body.coveredServiceTags !== undefined)
        update.coveredServiceTags = body.coveredServiceTags ?? [];
    ...
    if (body.isAmendment !== undefined)
        update.isAmendment = body.isAmendment;
    if (body.amendmentNumber !== undefined)
        update.amendmentNumber = body.amendmentNumber ?? null;
    if (body.executedDate !== undefined)
        update.executedDate = parseDate(body.executedDate);
    if (body.signedByCustomer !== undefined)
        update.signedByCustomer = body.signedByCustomer;
    if (body.signedByProvider !== undefined)
        update.signedByProvider = body.signedByProvider;
    if (body.signedAtCustomer !== undefined)
        update.signedAtCustomer = parseDate(body.signedAtCustomer);
    if (body.signedAtProvider !== undefined)
        update.signedAtProvider = parseDate(body.signedAtProvider);
...
slasRouter.delete('/:id', async (req, res) => {
    ...
    if (!ObjectId.isValid(id))
        return res.status(400).json({ data: null, error: 'invalid_id' });
    const result = await db.collection('sla_contracts').deleteOne({ _id: new ObjectId(id) });
...
slasRouter.post('/:id/amend', async (req, res) => {
    ...
    const amendmentNumber = typeof body.amendmentNumber === 'number' ? body.amendmentNumber : (existing.amendmentNumber ?? 0) + 1;
    const derived = {
        ...existing,
        _id: newId,
        contractNumber,
    ...
    if (body.effectiveDate !== undefined)
        patch.effectiveDate = parseDate(body.effectiveDate);
    if (body.startDate !== undefined)
        patch.startDate = parseDate(body.startDate);
    if (body.endDate !== undefined)
        patch.endDate = parseDate(body.endDate);
    if (body.autoRenew !== undefined)
        patch.autoRenew = body.autoRenew;
    if (body.renewalDate !== undefined)
        patch.renewalDate = parseDate(body.renewalDate);
    if (body.renewalTermMonths !== undefined)
        patch.renewalTermMonths = body.renewalTermMonths ?? null;
    if (body.noticePeriodDays !== undefined)
        patch.noticePeriodDays = body.noticePeriodDays ?? null;
    if (body.responseTargetMinutes !== undefined)
        patch.responseTargetMinutes = body.responseTargetMinutes ?? null;
    if (body.resolutionTargetMinutes !== undefined)
        patch.resolutionTargetMinutes = body.resolutionTargetMinutes ?? null;
    if (body.entitlements !== undefined)
        patch.entitlements = body.entitlements;
    if (body.notes !== undefined)
        patch.notes = body.notes;
    if (body.severityTargets !== undefined)
        patch.severityTargets = body.severityTargets;
    if (body.slaProfiles !== undefined)
        patch.slaProfiles = body.slaProfiles;
    if (body.customerLegalName !== undefined)
        patch.customerLegalName = body.customerLegalName;
    if (body.customerAddress !== undefined)
        patch.customerAddress = body.customerAddress;
    if (body.providerLegalName !== undefined)
        patch.providerLegalName = body.providerLegalName;
    if (body.providerAddress !== undefined)
        patch.providerAddress = body.providerAddress;
    if (body.counterpartyContactId !== undefined) {
    ...
    if (body.governingLaw !== undefined)
        patch.governingLaw = body.governingLaw;
    if (body.jurisdiction !== undefined)
        patch.jurisdiction = body.jurisdiction;
    if (body.paymentTerms !== undefined)
        patch.paymentTerms = body.paymentTerms;
    if (body.serviceScopeSummary !== undefined)
        patch.serviceScopeSummary = body.serviceScopeSummary;
    if (body.limitationOfLiability !== undefined)
        patch.limitationOfLiability = body.limitationOfLiability;
    if (body.indemnificationSummary !== undefined)
        patch.indemnificationSummary = body.indemnificationSummary;
    if (body.confidentialitySummary !== undefined)
        patch.confidentialitySummary = body.confidentialitySummary;
    if (body.dataProtectionSummary !== undefined)
        patch.dataProtectionSummary = body.dataProtectionSummary;
    if (body.ipOwnershipSummary !== undefined)
        patch.ipOwnershipSummary = body.ipOwnershipSummary;
    if (body.terminationConditions !== undefined)
        patch.terminationConditions = body.terminationConditions;
    if (body.changeOrderProcess !== undefined)
        patch.changeOrderProcess = body.changeOrderProcess;
    const finalDoc = { ...derived, ...patch };
...
slasRouter.post('/:id/render', async (req, res) => {
    ...
    if (body.templateId) {
        const tplColl = db.collection('contract_templates');
        const tpl = await tplColl.findOne({ _id: new ObjectId(body.templateId) });
        if (!tpl)
            return res.status(404).json({ data: null, error: 'template_not_found' });
...
slasRouter.post('/:id/send', async (req, res) => {
    ...
    if (body.templateId) {
        ...
        if (ctx.effectiveDate)
            parts.push(`Effective: ${new Date(ctx.effectiveDate).toLocaleDateString()}`);
    ...
    const user = req.user;
    const sentBy = user?.sub && ObjectId.isValid(user.sub)
...
export async function createSignatureInvites({ contractId, invites, createdByUserId, }) {
    const db = await getDb();
    if (!db)
...
slasRouter.post('/:id/signature-invites', async (req, res) => {
    ...
    const body = parsed.data;
    const user = req.user;
    const creatorId = user?.sub && ObjectId.isValid(user.sub)
        ? new ObjectId(user.sub)
        : undefined;
...
slasRouter.post('/:id/send-signed', async (req, res) => {
    ...
    await coll.updateOne({ _id: effectiveContract._id }, {
        $set: { updatedAt: new Date() },
        $push: { emailSends: emailEntry },
    });
    res.json({ data: { ok: true }, error: null });
