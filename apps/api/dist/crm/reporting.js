...
reportingRouter.get('/report', async (req, res) => {
    ...
    const invoicedDocs = (await db
        .collection('invoices')
        .find({ issuedAt: { $gte: start, $lt: endExclusive } }, { projection: { subtotal: 1, tax: 1, discountAmount: 1, total: 1 } })
        .limit(5000)
        .toArray());
    const invoiced = {
    ...
    }, { projection: { invoiceNumber: 1, title: 1, accountId: 1, balance: 1, dueDate: 1, issuedAt: 1, status: 1 } })
        .sort({ dueDate: 1 })
        .limit(50)
        .toArray());
    const accountIds = Array.from(new Set(overdueInvoices
        .map((x) => x.accountId)
        .filter(Boolean)
        .map((id) => String(id))));
    const accountDocs = accountIds.length > 0
        ? (await db
            .collection('accounts')
            .find({ _id: { $in: accountIds.filter(ObjectId.isValid).map((id) => new ObjectId(id)) } }, { projection: { name: 1, accountNumber: 1 } })
            .toArray())
        : [];
    ...
    const overdueInvoiceRows = overdueInvoices.map((inv) => {
        ...
        const daysOverdue = dueStart && Number.isFinite(dueStart.getTime()) ? Math.max(0, Math.floor((todayStart.getTime() - dueStart.getTime()) / msDay)) : null;
        return {
    ...
    const renewalsDue = (await db
        .collection('renewals')
        .find({ renewalDate: { $gte: start, $lt: endExclusive }, status: { $in: ['Active', 'Pending Renewal'] } }, { projection: { name: 1, accountName: 1, status: 1, renewalDate: 1, mrr: 1, arr: 1, churnRisk: 1 } })
        .sort({ renewalDate: 1 })
        .limit(50)
        .toArray());
    const highRiskRenewals = (await db
        .collection('renewals')
        .find({ churnRisk: 'High', status: { $in: ['Active', 'Pending Renewal'] } }, { projection: { name: 1, accountName: 1, status: 1, renewalDate: 1, mrr: 1, arr: 1, churnRisk: 1 } })
        .sort({ renewalDate: 1 })
        .limit(50)
        .toArray());
    // === Ticket backlog ===
    const backlog = (await db
        .collection('support_tickets')
        .find({ status: { $in: ['open', 'in_progress'] } }, { projection: { ticketNumber: 1, shortDescription: 1, priority: 1, status: 1, slaDueAt: 1, updatedAt: 1, requesterName: 1, requesterEmail: 1 } })
