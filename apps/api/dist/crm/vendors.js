...
vendorsRouter.get('/options', async (req, res) => {
    ...
    const items = await db
        .collection('crm_vendors')
        .find(filter, { projection: { name: 1, website: 1, status: 1 } })
        .sort({ name: 1 })
        .limit(500)
        .toArray();
    res.json({
...
vendorsRouter.post('/', async (req, res) => {
    ...
    await db.collection('crm_vendors').insertOne(doc);
...
vendorsRouter.put('/:id', async (req, res) => {
    ...
    const coll = db.collection('crm_vendors');
...
vendorsRouter.delete('/:id', async (req, res) => {
    ...
    const result = await db.collection('crm_vendors').deleteOne({ _id });
