...
kbRouter.get('/kb', async (req, res) => {
    ...
    const filter = {};
    if (q)
        filter.$or = [{ title: { $regex: q, $options: 'i' } }, { body: { $regex: q, $options: 'i' } }];
    if (tag)
        filter.tags = tag;
    if (category)
...
// GET /api/crm/support/kb/:idOrSlug
kbRouter.get('/kb/:idOrSlug', async (req, res) => {
    const db = await getDb();
    if (!db)
    ...
    if (ObjectId.isValid(idOrSlug)) {
        try {
            const _id = new ObjectId(idOrSlug);
    ...
    // Try as slug
    const item = await db.collection('kb_articles').findOne({ slug: idOrSlug });
    if (!item)
...
kbRouter.get('/kb/:id/attachments/:attId', async (req, res) => {
    ...
    try {
        ...
        const art = await db.collection('kb_articles').findOne({ _id }, { projection: { attachments: 1 } });
        const att = (art?.attachments || []).find((a) => String(a._id) === attId);
        if (!att)
            return res.status(404).json({ data: null, error: 'not_found' });
...
kbRouter.delete('/kb/:id/attachments/:attId', async (req, res) => {
    ...
    try {
        ...
        const attId = req.params.attId;
        const coll = db.collection('kb_articles');
        const art = await coll.findOne({ _id }, { projection: { attachments: 1 } });
        const att = (art?.attachments || []).find((a) => String(a._id) === attId);
        ...
        const pullId = att?._id ?? attId;
        await coll.updateOne({ _id }, { $pull: { attachments: { _id: pullId } }, $set: { updatedAt: new Date() } });
        res.json({ data: { ok: true }, error: null });
    }
    catch {
        res.status(400).json({ data: null, error: 'invalid_id' });
