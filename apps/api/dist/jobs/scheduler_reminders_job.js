    ...
    setInterval(async () => {
        try {
            ...
            for (const a of candidates) {
                ...
                if (!attendeeEmail)
                    continue;
                if (a.attendeeContactPreference && a.attendeeContactPreference !== 'email')
                    continue;
                ...
                try {
                    if (a.appointmentTypeId && ObjectId.isValid(String(a.appointmentTypeId))) {
                        const t = await db.collection('scheduler_appointment_types').findOne({ _id: new ObjectId(String(a.appointmentTypeId)) });
                        if (t?.name)
                ...
                await db.collection('appointments').updateOne({ _id: a._id }, { $set: { reminderEmailSentAt: new Date(), updatedAt: new Date() } });
            }
        }
        catch {
            // never throw
