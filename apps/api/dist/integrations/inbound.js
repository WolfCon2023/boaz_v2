...
async function logInbound(db, req, payload, result) {
    try {
...
inboundIntegrationsRouter.post('/accounts', async (req, res) => {
    ...
    try {
        ...
        const r = await db.collection('accounts').updateOne({ externalSource, externalId }, { $set: update, $setOnInsert: { createdAt: now } }, { upsert: true });
        const id = r.upsertedId
            ? String(r.upsertedId._id)
            : String((await db.collection('accounts').findOne({ externalSource, externalId }, { projection: { _id: 1 } }))?._id);
...
inboundIntegrationsRouter.post('/contacts', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const raw = req.body ?? {};
    try {
        ...
        if (accSource && accId) {
            const acc = await db
                .collection('accounts')
                .findOne({ externalSource: accSource, externalId: accId }, { projection: { _id: 1 } });
            if (acc?._id)
                accountId = acc._id;
        }
        const update = {
        ...
        const id = r.upsertedId
            ? String(r.upsertedId._id)
            : String((await db.collection('contacts').findOne(filter, { projection: { _id: 1 } }))?._id);
...
inboundIntegrationsRouter.post('/deals', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const raw = req.body ?? {};
    try {
        ...
        Object.keys(update).forEach((k) => update[k] === undefined && delete update[k]);
        const r = await db.collection('deals').updateOne({ externalSource, externalId }, { $set: update, $setOnInsert: { createdAt: now, stageChangedAt: now } }, { upsert: true });
        const id = r.upsertedId
            ? String(r.upsertedId._id)
            : String((await db.collection('deals').findOne({ externalSource, externalId }, { projection: { _id: 1 } }))?._id);
...
inboundIntegrationsRouter.post('/tickets', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const raw = req.body ?? {};
    try {
        ...
        Object.keys(update).forEach((k) => update[k] === undefined && delete update[k]);
        const r = await db.collection('support_tickets').updateOne({ externalSource, externalId }, { $set: update, $setOnInsert: { createdAt: now, comments: [] } }, { upsert: true });
        const id = r.upsertedId
            ? String(r.upsertedId._id)
            : String((await db.collection('support_tickets').findOne({ externalSource, externalId }, { projection: { _id: 1 } }))?._id);
