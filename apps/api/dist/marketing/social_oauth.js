    ...
    try {
        switch (platform) {
            ...
            case 'instagram':
                ...
                // Get user's pages
                const fbPagesResponse = await fetch(`https://graph.facebook.com/v18.0/me/accounts?access_token=${accessToken}`);
            ...
            case 'twitter':
                ...
                const twitterTokenData = await twitterTokenResponse.json();
...
socialOAuthRouter.post('/refresh/:accountId', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const account = await db.collection('social_accounts').findOne({ _id: accountId });
        if (!account) {
        ...
        await db.collection('social_accounts').updateOne({ _id: accountId }, {
            $set: {
