...
async function ensureFinancialIndexes(db) {
    try {
...
async function getNextSequence(db, name, startAt = 1) {
...
financialRouter.get('/chart-of-accounts', async (req, res) => {
...
financialRouter.get('/chart-of-accounts/:id', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
...
financialRouter.post('/chart-of-accounts', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
    ...
    if (!parsed.success)
        return res.status(400).json({ data: null, error: 'invalid_input', details: parsed.error.flatten() });
    ...
    if (existing)
        return res.status(400).json({ data: null, error: 'account_number_exists' });
    const now = new Date();
    const doc = {
        _id: new ObjectId(),
        accountNumber,
    ...
    await db.collection('fi_chart_of_accounts').insertOne(doc);
    res.status(201).json({
        data: {
            id: String(doc._id),
...
financialRouter.patch('/chart-of-accounts/:id', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    if (!account)
        return res.status(404).json({ data: null, error: 'not_found' });
    const updates = { updatedAt: new Date(), updatedBy: auth.userId };
    if (typeof req.body.name === 'string')
        updates.name = req.body.name;
    if (typeof req.body.description === 'string' || req.body.description === null)
        updates.description = req.body.description;
    if (typeof req.body.taxCode === 'string' || req.body.taxCode === null)
        updates.taxCode = req.body.taxCode;
    if (typeof req.body.isActive === 'boolean')
        updates.isActive = req.body.isActive;
    if (typeof req.body.subType === 'string')
        updates.subType = req.body.subType;
...
financialRouter.post('/chart-of-accounts/seed-default', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
...
financialRouter.get('/periods', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
...
financialRouter.post('/periods', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
    ...
    if (existing)
        return res.status(400).json({ data: null, error: 'period_exists' });
    const now = new Date();
    const doc = {
        _id: new ObjectId(),
        name,
    ...
    await db.collection('fi_periods').insertOne(doc);
    res.status(201).json({
        data: {
            id: String(doc._id),
            name: doc.name,
...
financialRouter.post('/periods/generate-year', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
...
financialRouter.patch('/periods/:id/close', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    const now = new Date();
    await db.collection('fi_periods').updateOne({ _id: id }, { $set: { status: 'closed', closedAt: now, closedBy: auth.userId, updatedAt: now } });
    res.json({ data: { id: String(id), status: 'closed' }, error: null });
});
// PATCH /api/financial/periods/:id/reopen (Admin only)
financialRouter.patch('/periods/:id/reopen', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    if (period.status === 'locked')
        return res.status(400).json({ data: null, error: 'period_locked' });
    const now = new Date();
    await db.collection('fi_periods').updateOne({ _id: id }, { $set: { status: 'open', closedAt: null, closedBy: null, updatedAt: now } });
...
financialRouter.get('/journal-entries', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
    ...
    res.json({
        data: {
            items: entries.map((e) => ({
                ...
                lines: e.lines,
                status: e.status,
                totalDebits: e.lines.reduce((sum, l) => sum + (l.debit || 0), 0),
                totalCredits: e.lines.reduce((sum, l) => sum + (l.credit || 0), 0),
...
financialRouter.get('/journal-entries/:id', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const id = objIdOrNull(req.params.id);
...
financialRouter.post('/journal-entries', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureFinancialIndexes(db);
    ...
    const accountIds = lines.map(l => objIdOrNull(l.accountId)).filter((id) => id !== null);
    ...
    await db.collection('fi_journal_entries').insertOne(doc);
    res.status(201).json({
        data: {
            id: String(doc._id),
...
financialRouter.post('/journal-entries/:id/reverse', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    if (entry.status === 'reversed')
        return res.status(400).json({ data: null, error: 'already_reversed' });
    if (entry.status !== 'posted')
        return res.status(400).json({ data: null, error: 'can_only_reverse_posted' });
    ...
    await db.collection('fi_journal_entries').insertOne(reversalDoc);
    // Mark original as reversed
    await db.collection('fi_journal_entries').updateOne({ _id: id }, {
...
financialRouter.get('/trial-balance', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    ...
    const accountIds = balances.map((b) => b._id).filter(Boolean);
    const accounts = await db.collection('fi_chart_of_accounts').find({ _id: { $in: accountIds } }).toArray();
    const accountMap = new Map(accounts.map((a) => [String(a._id), a]));
    ...
    const totalDebits = trialBalance.reduce((sum, b) => sum + b.totalDebits, 0);
    const totalCredits = trialBalance.reduce((sum, b) => sum + b.totalCredits, 0);
    res.json({
        data: {
            asOfDate,
...
financialRouter.get('/income-statement', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
...
financialRouter.get('/balance-sheet', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const asOfDate = req.query.asOfDate ? new Date(req.query.asOfDate) : new Date();
...
financialRouter.get('/cash-flow-statement', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const startDate = req.query.startDate ? new Date(req.query.startDate) : new Date(new Date().getFullYear(), 0, 1);
    ...
    for (const r of workingCapitalResults) {
        ...
        switch (r._id.accountNumber) {
            case '1100':
                arChange = -netChange;
                break; // Decrease in AR is + cash
            case '2000':
                apChange = -netChange;
                break; // AP: Credit - Debit (increase in AP is + cash)
            case '2100':
                accruedExpenseChange = -netChange;
                break;
            case '2200':
                deferredRevenueChange = -netChange;
                break;
        }
    }
...
financialRouter.get('/account-drill-down/:accountId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    ...
    const transactions = entries.reverse().map((e) => {
        const relevantLines = e.lines.filter((l) => String(l.accountId) === String(accountId));
        const debit = relevantLines.reduce((sum, l) => sum + (l.debit || 0), 0);
        const credit = relevantLines.reduce((sum, l) => sum + (l.credit || 0), 0);
        // Calculate balance based on normal balance
        const netChange = account.normalBalance === 'Debit' ? (debit - credit) : (credit - debit);
...
// ============================================================================
import { postInvoiceCreated, postPaymentReceived, postTimeEntry, postRenewalCreated, journalEntryExistsForSource, } from './auto_posting.js';
// POST /api/financial/auto-post/invoices - Retroactively post all unposted invoices
financialRouter.post('/auto-post/invoices', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    for (const inv of invoices) {
        ...
        const result = await postInvoiceCreated(db, String(inv._id), inv.invoiceNumber || 'N/A', inv.total, inv.issueDate || inv.createdAt || new Date(), customerName, auth.userId, auth.email);
        if (result) {
            posted++;
        }
        else {
...
financialRouter.post('/auto-post/payments', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    for (const inv of invoices) {
        ...
        for (const payment of (inv.payments || [])) {
            ...
            const result = await postPaymentReceived(db, paymentId, String(inv._id), inv.invoiceNumber || 'N/A', payment.amount, payment.date ? new Date(payment.date) : new Date(), customerName, payment.method, auth.userId, auth.email);
            if (result) {
                posted++;
            }
            else {
...
financialRouter.post('/auto-post/time-entries', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    for (const entry of timeEntries) {
        ...
        const result = await postTimeEntry(db, String(entry._id), String(entry.projectId), projectName, entry.billable === true, entry.minutes, defaultHourlyRate, entry.workDate || entry.createdAt || new Date(), userName, auth.userId, auth.email);
        if (result) {
            posted++;
        }
        else {
...
financialRouter.post('/auto-post/renewals', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    for (const renewal of renewals) {
        ...
        const result = await postRenewalCreated(db, String(renewal._id), renewal.amount, renewal.startDate || renewal.createdAt || new Date(), customerName, productName, renewal.term, auth.userId, auth.email);
        if (result) {
            posted++;
        }
        else {
...
financialRouter.post('/auto-post/all', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
...
financialRouter.post('/expenses', async (req, res) => {
    ...
    for (const line of lines) {
        ...
        if (!account)
            return res.status(400).json({ data: null, error: 'account_not_found', accountId: line.accountId });
        enrichedLines.push({
    ...
    await db.collection('fi_expenses').insertOne(doc);
    res.status(201).json({
        data: {
            id: String(doc._id),
...
financialRouter.patch('/expenses/:id', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    if (expense.status === 'paid' || expense.status === 'void') {
    ...
    if (typeof body.vendorName === 'string')
        updates.vendorName = body.vendorName;
    if (body.date)
    ...
    if (typeof body.receipt === 'string' || body.receipt === null)
        updates.receipt = body.receipt;
    await db.collection('fi_expenses').updateOne({ _id: id }, { $set: updates });
    res.json({ data: { id: String(id), updated: true }, error: null });
});
// POST /api/financial/expenses/:id/approve
financialRouter.post('/expenses/:id/approve', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    const now = new Date();
    await db.collection('fi_expenses').updateOne({ _id: id }, { $set: { status: 'approved', approvedBy: auth.userId, approvedAt: now, updatedAt: now, updatedBy: auth.userId } });
...
financialRouter.post('/expenses/:id/pay', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    await db.collection('fi_expenses').updateOne({ _id: id }, {
        $set: {
            status: 'paid',
            paidAt: now,
...
financialRouter.post('/expenses/:id/void', requirePermission('*'), async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const auth = req.auth;
    ...
    const now = new Date();
    await db.collection('fi_expenses').updateOne({ _id: id }, { $set: { status: 'void', updatedAt: now, updatedBy: auth.userId } });
    res.json({ data: { id: String(id), status: 'void' }, error: null });
});
// GET /api/financial/expense-categories - Get common expense categories
financialRouter.get('/expense-categories', async (req, res) => {
    const categories = [
...
financialRouter.get('/analytics/kpis', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    ...
    let ytdOpex = 0;
    let ytdOtherExp = 0;
    for (const r of ytdIncomeExpense) {
    ...
    let inventory = 0;
    for (const r of balanceSheetData) {
...
financialRouter.get('/analytics/anomalies', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    ...
    for (const e of entries) {
        const total = e.lines.reduce((sum, l) => sum + (l.debit || 0), 0);
        const src = e.sourceType;
        if (!statsBySource.has(src))
            statsBySource.set(src, { count: 0, sum: 0, values: [] });
...
financialRouter.get('/analytics/profitability-by-project', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    const startDate = req.query.startDate ? new Date(req.query.startDate) : new Date(new Date().getFullYear(), 0, 1);
    ...
    for (const inv of invoicesByProject) {
        if (inv._id) {
            const key = String(inv._id);
            if (!projectMap.has(key))
                projectMap.set(key, { revenue: 0, laborCost: 0, invoiceCount: 0 });
            const entry = projectMap.get(key);
            entry.revenue = inv.revenue;
            entry.invoiceCount = inv.invoiceCount;
        }
    }
    for (const lab of laborByProject) {
        if (lab._id) {
    ...
    const totals = profitabilityData.reduce((acc, p) => ({
