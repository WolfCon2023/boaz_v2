...
import { z } from 'zod';
import { getUserByEmail, verifyCredentials, verifySecurityAnswer, getRandomSecurityQuestion, createPasswordResetToken, resetPasswordWithToken, verifyEnrollmentToken, updateSecurityQuestions, completeEnrollment, getUserById, getUserSecurityQuestions, updateUserProfile, createUserByAdmin, changePassword, resetUserToTemporaryPassword, createRegistrationRequest, getRegistrationRequests, approveRegistrationRequest, rejectRegistrationRequest, getUserApplicationAccess, hasApplicationAccess, grantApplicationAccess, revokeApplicationAccess, getUserAccessList, APPLICATION_CATALOG, createApplicationAccessRequest, getApplicationAccessRequests, approveApplicationAccessRequest, rejectApplicationAccessRequest, getUserApplicationAccessRequests, getApplicationAccessRequestById, generateAccessReport, } from './store.js';
import { hasEmailNotificationsEnabled } from './preferences-helper.js';
import { signAccessToken, signRefreshToken, verifyAny } from './jwt.js';
...
authRouter.post('/admin/users', requireAuth, requirePermission('*'), async (req, res) => {
    try {
        ...
        const { user, temporaryPassword } = await createUserByAdmin(parsed.data.email, parsed.data.name, parsed.data.phoneNumber, parsed.data.workLocation);
...
authRouter.post('/me/change-password', requireAuth, async (req, res) => {
    try {
        ...
        const auth = req.auth;
        const success = await changePassword(auth.userId, parsed.data.currentPassword, parsed.data.newPassword);
        if (!success) {
...
authRouter.get('/admin/users', requireAuth, requirePermission('*'), async (req, res) => {
    try {
        ...
        const roles = roleIds.length > 0
            ...
            : [];
        // Build role map
        const roleMap = new Map(roles.map((r) => [r._id.toString(), { id: r._id.toString(), name: r.name, permissions: r.permissions || [] }]));
...
authRouter.patch('/admin/users/:id/role', requireAuth, requirePermission('*'), async (req, res) => {
    try {
        ...
        await db.collection('user_roles').deleteMany({ userId: userIdStr });
        let assignedRole = null;
        if (parsed.data.roleId) {
            // Assign new role
            let roleId;
...
authRouter.get('/managers', requireAuth, async (req, res) => {
    try {
        ...
        const userRoles = await db.collection('user_roles').find({ roleId: managerRole._id }).toArray();
        const managerUserIds = userRoles.map((ur) => ur.userId);
        if (managerUserIds.length === 0) {
            return res.json({ managers: [] });
        }
        // Get user details
        const userIdObjects = managerUserIds.map((id) => {
            try {
