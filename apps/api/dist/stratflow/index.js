...
stratflowRouter.get('/sprints/:sprintId/burndown', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    const currentIssues = await db.collection('sf_issues').find({ sprintId: sid }).toArray();
...
stratflowRouter.get('/projects/:projectId/velocity', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    const closedSprints = await db
        ...
        .find({ projectId: pid, state: 'closed' })
    ...
    for (const sprint of closedSprints) {
        ...
        const sprintIssues = await db
            .collection('sf_issues')
            .find({ $or: [{ sprintId: sprint._id }, { epicId: { $exists: true } }] })
            .toArray();
        ...
        const completedIssues = completedIssueIds.length
            ? await db
                .collection('sf_issues')
                .find({ _id: { $in: completedIssueIds.map((id) => objIdOrNull(id)).filter(Boolean) } })
                .toArray()
            : [];
        ...
        const totalIssueCount = await db.collection('sf_issues').countDocuments({ sprintId: sprint._id });
        velocityData.push({
...
stratflowRouter.get('/projects/:projectId/workload', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    const activeSprint = await db.collection('sf_sprints').findOne({ projectId: pid, state: 'active' });
    // Get all open issues (not done) for this project
    const openIssues = await db
        .collection('sf_issues')
        .find({ projectId: pid, statusKey: { $ne: 'done' } })
        .toArray();
    // Group by assignee
...
stratflowRouter.get('/projects/:projectId/templates', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (project === 'forbidden')
        return res.status(403).json({ data: null, error: 'forbidden' });
    const items = await db.collection('sf_issue_templates').find({ projectId: pid }).sort({ name: 1 }).toArray();
    res.json({
        data: {
            items: items.map((t) => ({
                ...t,
...
stratflowRouter.post('/projects/:projectId/templates', async (req, res) => {
    ...
    await db.collection('sf_issue_templates').insertOne(doc);
    res.status(201).json({ data: { _id: String(doc._id) }, error: null });
});
// DELETE /api/stratflow/templates/:templateId
stratflowRouter.delete('/templates/:templateId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (!tid)
        return res.status(400).json({ data: null, error: 'invalid_template_id' });
    const template = await db.collection('sf_issue_templates').findOne({ _id: tid });
    if (!template)
        return res.status(404).json({ data: null, error: 'not_found' });
    const project = await loadProjectForUser(db, template.projectId, auth.userId);
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    await db.collection('sf_issue_templates').deleteOne({ _id: tid });
    res.json({ data: { ok: true }, error: null });
});
// POST /api/stratflow/issues/:issueId/clone - Clone an issue
stratflowRouter.post('/issues/:issueId/clone', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (!project)
        return res.status(404).json({ data: null, error: 'not_found' });
    const now = new Date();
    const lastIssue = await db.collection('sf_issues').findOne({ columnId: issue.columnId }, { sort: { order: -1 } });
    ...
    await db.collection('sf_issues').insertOne(cloned);
...
stratflowRouter.get('/projects/:projectId/releases', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (project === 'forbidden')
        return res.status(403).json({ data: null, error: 'forbidden' });
    const items = await db.collection('sf_releases').find({ projectId: pid }).sort({ targetDate: -1, createdAt: -1 }).toArray();
    res.json({
        data: {
            items: items.map((r) => ({
                ...r,
                _id: String(r._id),
                projectId: String(r.projectId),
                sprintIds: (r.sprintIds || []).map((s) => String(s)),
...
stratflowRouter.post('/projects/:projectId/releases', async (req, res) => {
    ...
    const doc = {
        ...
        description: parsed.data.description || null,
        state: (parsed.data.state || 'planned'),
        targetDate: parsed.data.targetDate ? new Date(parsed.data.targetDate) : null,
        releaseDate: parsed.data.releaseDate ? new Date(parsed.data.releaseDate) : null,
        sprintIds: (parsed.data.sprintIds || []).map((s) => objIdOrNull(s)).filter(Boolean),
        createdAt: now,
        updatedAt: now,
    };
    await db.collection('sf_releases').insertOne(doc);
    res.status(201).json({ data: { _id: String(doc._id) }, error: null });
});
// PATCH /api/stratflow/releases/:releaseId
stratflowRouter.patch('/releases/:releaseId', async (req, res) => {
    ...
    if (!rid)
        return res.status(400).json({ data: null, error: 'invalid_release_id' });
    const release = await db.collection('sf_releases').findOne({ _id: rid });
    if (!release)
        return res.status(404).json({ data: null, error: 'not_found' });
    const project = await loadProjectForUser(db, release.projectId, auth.userId);
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    ...
    if (parsed.data.version !== undefined)
        update.version = parsed.data.version.trim();
    if (parsed.data.description !== undefined)
        update.description = parsed.data.description || null;
    if (parsed.data.state !== undefined)
        update.state = parsed.data.state;
    if (parsed.data.targetDate !== undefined)
        update.targetDate = parsed.data.targetDate ? new Date(parsed.data.targetDate) : null;
    if (parsed.data.releaseDate !== undefined)
        update.releaseDate = parsed.data.releaseDate ? new Date(parsed.data.releaseDate) : null;
    if (parsed.data.sprintIds !== undefined)
        update.sprintIds = (parsed.data.sprintIds || []).map((s) => objIdOrNull(s)).filter(Boolean);
    await db.collection('sf_releases').updateOne({ _id: rid }, { $set: update });
    res.json({ data: { ok: true }, error: null });
});
// DELETE /api/stratflow/releases/:releaseId
stratflowRouter.delete('/releases/:releaseId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    await db.collection('sf_releases').deleteOne({ _id: rid });
    res.json({ data: { ok: true }, error: null });
});
// ============================================================================
// WEBHOOKS
...
stratflowRouter.get('/projects/:projectId/webhooks', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    const items = await db.collection('sf_webhooks').find({ projectId: pid }).sort({ createdAt: -1 }).toArray();
    res.json({
        data: {
            items: items.map((w) => ({
                _id: String(w._id),
...
stratflowRouter.post('/projects/:projectId/webhooks', async (req, res) => {
    ...
    await db.collection('sf_webhooks').insertOne(doc);
    res.status(201).json({ data: { _id: String(doc._id) }, error: null });
});
// PATCH /api/stratflow/webhooks/:webhookId
stratflowRouter.patch('/webhooks/:webhookId', async (req, res) => {
    ...
    if (!wid)
        return res.status(400).json({ data: null, error: 'invalid_webhook_id' });
    const webhook = await db.collection('sf_webhooks').findOne({ _id: wid });
    if (!webhook)
        return res.status(404).json({ data: null, error: 'not_found' });
    const project = await loadProjectForUser(db, webhook.projectId, auth.userId);
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    ...
    if (parsed.data.events !== undefined)
        update.events = parsed.data.events;
    if (parsed.data.enabled !== undefined)
        update.enabled = Boolean(parsed.data.enabled);
    await db.collection('sf_webhooks').updateOne({ _id: wid }, { $set: update });
    res.json({ data: { ok: true }, error: null });
});
// DELETE /api/stratflow/webhooks/:webhookId
stratflowRouter.delete('/webhooks/:webhookId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    await db.collection('sf_webhooks').deleteOne({ _id: wid });
    res.json({ data: { ok: true }, error: null });
});
// ============================================================================
// CUSTOM FIELDS
...
stratflowRouter.get('/projects/:projectId/custom-fields', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (project === 'forbidden')
        return res.status(403).json({ data: null, error: 'forbidden' });
    const items = await db.collection('sf_custom_fields').find({ projectId: pid }).sort({ name: 1 }).toArray();
    res.json({
        data: {
            items: items.map((f) => ({
                _id: String(f._id),
...
stratflowRouter.post('/projects/:projectId/custom-fields', async (req, res) => {
    ...
    try {
        await db.collection('sf_custom_fields').insertOne(doc);
    }
    catch {
...
stratflowRouter.delete('/custom-fields/:fieldId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (!fid)
        return res.status(400).json({ data: null, error: 'invalid_field_id' });
    const field = await db.collection('sf_custom_fields').findOne({ _id: fid });
    if (!field)
        return res.status(404).json({ data: null, error: 'not_found' });
    const project = await loadProjectForUser(db, field.projectId, auth.userId);
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    if (String(project.ownerId) !== auth.userId)
        return res.status(403).json({ data: null, error: 'owner_only' });
    await db.collection('sf_custom_fields').deleteOne({ _id: fid });
    res.json({ data: { ok: true }, error: null });
});
// ============================================================================
...
stratflowRouter.get('/sprints/:sprintId/retro', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (!project)
        return res.status(404).json({ data: null, error: 'not_found' });
    const items = await db.collection('sf_retro_items').find({ sprintId: sid }).sort({ createdAt: 1 }).toArray();
    // Load authors
    const authorIds = [...new Set(items.map((i) => String(i.authorId)).filter(Boolean))];
...
stratflowRouter.post('/sprints/:sprintId/retro', async (req, res) => {
    ...
    await db.collection('sf_retro_items').insertOne(doc);
    res.status(201).json({ data: { _id: String(doc._id) }, error: null });
});
// POST /api/stratflow/retro/:itemId/vote
stratflowRouter.post('/retro/:itemId/vote', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (!iid)
        return res.status(400).json({ data: null, error: 'invalid_item_id' });
    const item = await db.collection('sf_retro_items').findOne({ _id: iid });
    if (!item)
        return res.status(404).json({ data: null, error: 'not_found' });
    const project = await loadProjectForUser(db, item.projectId, auth.userId);
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    const votes = item.votes || [];
    if (votes.includes(auth.userId)) {
        // Un-vote
        await db.collection('sf_retro_items').updateOne({ _id: iid }, { $pull: { votes: auth.userId }, $set: { updatedAt: new Date() } });
    }
    else {
        // Vote
        await db.collection('sf_retro_items').updateOne({ _id: iid }, { $addToSet: { votes: auth.userId }, $set: { updatedAt: new Date() } });
    }
    res.json({ data: { ok: true }, error: null });
});
// POST /api/stratflow/retro/:itemId/resolve
stratflowRouter.post('/retro/:itemId/resolve', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    if (project === 'forbidden' || !project)
        return res.status(403).json({ data: null, error: 'forbidden' });
    await db.collection('sf_retro_items').updateOne({ _id: iid }, { $set: { resolved: !item.resolved, updatedAt: new Date() } });
    res.json({ data: { ok: true }, error: null });
});
// DELETE /api/stratflow/retro/:itemId
stratflowRouter.delete('/retro/:itemId', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    await db.collection('sf_retro_items').deleteOne({ _id: iid });
    res.json({ data: { ok: true }, error: null });
});
// ============================================================================
// AUDIT LOG
...
async function logAudit(db, projectId, actorId, action, targetType, targetId, changes, ipAddress, userAgent) {
    const doc = {
        _id: new ObjectId(),
        projectId,
        actorId,
    ...
    await db.collection('sf_audit_log').insertOne(doc);
}
// GET /api/stratflow/projects/:projectId/audit-log
stratflowRouter.get('/projects/:projectId/audit-log', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
...
stratflowRouter.get('/projects/:projectId/financial-summary', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    const byUserAgg = await db
        ...
        .toArray();
    const userIds = byUserAgg.map((u) => u._id).filter(Boolean);
    const users = userIds.length ? await loadUsersByIds(db, userIds) : [];
    const userMap = new Map(users.map((u) => [u.id, u]));
...
stratflowRouter.get('/financial-analytics', async (req, res) => {
    const db = await getDb();
    if (!db)
        return res.status(500).json({ data: null, error: 'db_unavailable' });
    await ensureStratflowIndexes(db);
    ...
    const byProjectAgg = await db
        ...
        .toArray();
    const projectMap = new Map(userProjects.map((p) => [String(p._id), p]));
    ...
    if (weeklyAgg.length >= 2) {
        ...
        const firstAvg = firstHalf.reduce((s, w) => s + w.billableMinutes, 0) / (firstHalf.length || 1);
        const secondAvg = secondHalf.reduce((s, w) => s + w.billableMinutes, 0) / (secondHalf.length || 1);
    ...
    const avgWeeklyBillable = weeklyAgg.length > 0 ? weeklyAgg.reduce((s, w) => s + w.billableMinutes, 0) / weeklyAgg.length : 0;
